...
export async function createMember(formData: FormData) {
  ...
  try {
    ...
    console.log("[v0] getAdminSupabase error:", (e as Error)?.message)
    return {
      ok: false,
      error:
        "Supabase admin client not configured. Ensure SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are set (or NEXT_PUBLIC_* in dev).",
    }
  }
  ...
  try {
    ...
    // Return void on success to avoid server action response mismatch
    return
  } catch (err) {
    console.log("[v0] createMember: unexpected error:", err)
    return { ok: false, error: "Unexpected server error. Check server logs." }
  }
}
...
export async function createEvent(formData: FormData) {
  ...
  try {
    ...
    console.log("[v0] createEvent: unexpected error:", err)
    return { ok: false, error: "Unexpected server error. Check server logs." }
  }
}
